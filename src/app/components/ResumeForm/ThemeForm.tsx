...
export const ThemeForm = () => {
  ...
  const handleDownloadData = () => {
    const data = { resume, settings };
    const blob = new Blob([JSON.stringify(data, null, 2)], { type: "application/json" });
    const url = URL.createObjectURL(blob);
    const link = document.createElement("a");
    link.href = url;
    link.download = `open-resume-${resume.profile.name || "data"}.json`;
    link.click();
      ...
      <div className="flex flex-col gap-6">
        <div className="flex items-center justify-between">
          ...
          <button
            ...
            className="flex items-center gap-1.5 rounded-md bg-gray-100 px-3 py-1.5 text-[10px] font-bold uppercase text-gray-600 transition-colors hover:bg-gray-200"
          >
