...
export const FormTabs = ({ activeTab, onChange }: FormTabsProps) => {
  const tabs = [
    ...
    { id: "track", label: "Track", icon: BriefcaseIcon, isNew: false },
  ] as const;
  ...
  return (
    <div className="sticky top-0 z-30 border-b border-gray-200 bg-white/80 backdrop-blur-md dark:border-gray-800 dark:bg-gray-900/80">
      <nav className="flex justify-center gap-2 px-2 lg:gap-8" aria-label="Tabs">
        {tabs.map((tab) => {
          ...
          return (
            ...
            >
              ...
              {tab.isNew && (
                <span className="absolute -right-1 top-3 flex h-2 w-2 sm:-right-4">
